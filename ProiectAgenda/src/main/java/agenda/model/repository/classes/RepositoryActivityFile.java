...
public class RepositoryActivityFile implements RepositoryActivity{
	...
	private static final String filename = "C:\\Users\\Andrada\\Desktop\\facultate\\UltimulSemestru\\VVSS\\ProiectAgenda\\files\\activities.txt";
	...
	public boolean addActivity(Activity activity) {
		...
		while( i < activities.size() )
		{
			if ( activities.get(i).getStart().compareTo(activity.getStart()) < 0 ||
					activity.getDuration().compareTo(activities.get(i).getDuration()) < 0 )
			{
				conflicts = true;
			}

			//System.out.println(i + " " + activities.get(i).getStart() + " " + activities.get(i).getDuration());
